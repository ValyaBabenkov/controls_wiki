...
import { TypeEventSchema } from '@/src/schemas/events.schema'
...
const components = {
	...
	h3: ({ className, ...props }: HTMLAttributes<HTMLHeadingElement>) => (
		<h3
			className={cn(
				'font-heading mt-8 scroll-m-20 text-xl font-semibold tracking-normal',
				className
			)}
			{...props}
		/>
	...
	h4: ({ className, ...props }: HTMLAttributes<HTMLHeadingElement>) => (
		<h4
			className={cn(
				'font-heading mt-8 scroll-m-20 text-lg font-semibold tracking-normal',
				className
			)}
			{...props}
		/>
	...
	pre: ({
		...
		__src__?: string
		__event__?: TypeEventSchema['name']
	} & NpmCommands) => {
		return (
